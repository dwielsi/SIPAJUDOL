    ...
    <div x-data="{
            ...
            async destroy(row) {
                const result = await Swal.fire({
                    ...
                    text: row.report_number + ' akan dipindahkan ke arsip.',
                    icon: 'warning',
                ...
                if (res.ok) {
                    Swal.fire({ toast: true, position: 'top-end', icon: 'success', title: 'Laporan berhasil dihapus', showConfirmButton: false, timer: 3000 });
                    this.fetchRows();
                } else {
                    Swal.fire({ toast: true, position: 'top-end', icon: 'error', title: 'Gagal menghapus laporan', showConfirmButton: false, timer: 3000 });
                }
            }
         }"
         class="space-y-4">
        ...
        <x-card :padding="false">
            <div class="flex flex-col gap-3 border-b border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between dark:border-slate-800">
                ...
                @can('create', \App\Models\Report::class)
                    <x-button variant="primary" onclick="window.location='{{ route('reports.create') }}'">
                            ...
                            <tr class="transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                ...
                                <td class="px-4 py-3.5">
                                    <span class="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium"
                                          :class="{
                                              'bg-success-50 text-success-700 dark:bg-success-500/10 dark:text-success-500': row.status_badge.color === 'success',
                                              'bg-primary-50 text-primary-700 dark:bg-primary-500/10 dark:text-primary-400': row.status_badge.color === 'primary',
                                              'bg-slate-100 text-slate-600 dark:bg-slate-700/50 dark:text-slate-300': row.status_badge.color === 'slate',
                                          }"
                                          x-text="row.status_badge.label"></span>
                                </td>
                                <td class="px-4 py-3.5">
                                    <div class="flex items-center justify-end gap-1">
                                        ...
                                        <a x-show="row.actions.can_print" :href="row.actions.pdf_url" target="_blank" class="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-primary-600 dark:hover:bg-slate-700" title="Cetak PDF">
                                        ...
                                        <a x-show="row.actions.can_update" :href="row.actions.edit_url" class="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-primary-600 dark:hover:bg-slate-700" title="Ubah">
            ...
            <div class="flex flex-col items-center justify-between gap-3 border-t border-slate-200 p-4 text-sm sm:flex-row dark:border-slate-800" x-show="!loading && filtered > 0">
                <p class="text-slate-500 dark:text-slate-400">
                    ...
                    dari <span x-text="filtered"></span> laporan
                </p>
